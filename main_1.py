...
def catch_corona(wave_image, threshold=0.8):
    ...
    #Get doctor Position
    doctor = [(np.float16(r[0]) , np.float16(r[1]), max((np.float16(r[2]) , np.float16(r[3])))) for r in np_rs if r[5] == 0]

    #Get virus Position
    rs = [(np.float16(r[0]) , np.float16(r[1])) for r in np_rs if r[5] != 0 and r[5] != 2]

    #Remove Virus having distace to Doctor < max(doctor_h, doctor_w)
    rs_ = []
    for virus in rs:
        valid = True
        for doc in doctor:
            d = (doc[0], doc[1])
            dis = doc[2]
            if sum([abs(x) for x in map(operator.sub, d, virus)]) < float(dis) :
                valid = False
                break
            if valid:
                rs_.append(virus) 
